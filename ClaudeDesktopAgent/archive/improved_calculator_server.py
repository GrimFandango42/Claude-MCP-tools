...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(os.path.join(os.path.dirname(os.path.abspath(__file__)), "improved_calculator_server.log")),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger("improved_calculator_server")
...
def read_message():
    try:
        # Read the 4-byte header (uint32 big-endian)
        ...
        content_length = struct.unpack("!I", header)[0]  # Big-endian (network byte order)
...
def write_message(message):
    try:
        ...
        # Write header (4 bytes for content length, big-endian)
        header = struct.pack("!I", content_length)  # Big-endian (network byte order)
...
# Main function
def main():
    logger.info("Starting improved calculator MCP server")
