...
async def analyze_project(project_path: str) -> dict:
    ...
    try:
        ...
        # Use await instead of creating new event loop
        project_context = await claude_code_mcp._analyze_project(project_path)
...
@mcp.tool()
async def set_active_project(project_path: str) -> dict:
    """Set the active project path with automatic analysis"""
    try:
        if project_path not in claude_code_mcp.projects:
            analysis_result = await analyze_project(project_path)
...
async def delegate_coding_task(
    ...
    try:
        ...
        try:
            asyncio.create_task(claude_code_mcp._execute_task_async(task_id))
