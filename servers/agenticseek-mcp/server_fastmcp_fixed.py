#!/usr/bin/env python3
"""
AgenticSeek MCP Server - FastMCP Fixed Implementation
Multi-Provider AI Routing with Cost Optimization

FIXED: Removed asyncio.run() calls that conflict with Claude's event loop
...
logger.info("Using FastMCP for AgenticSeek MCP implementation (FIXED VERSION)")
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=60.0) as client:
            ...
            if result.get("success") == "true":
                ...
                metadata += f"Privacy: {'🔒 Local Processing' if provider_info.is_local else '☁️ Cloud Processing'}\\n"
...
async def switch_provider(provider_key: str):
    ...
    try:
        ...
        logger.error(f"Error in switch_provider: {e}")
...
# FIXED: Made tool functions async and use await instead of asyncio.run()
@mcp.tool()
async def local_reasoning(prompt: str, context: str = "") -> str:
    """
    ...
    """
    return await call_agenticseek(prompt, "local", context)

@mcp.tool()
async def openai_reasoning(prompt: str, model: str = "gpt-3.5-turbo", context: str = "") -> str:
    """
    ...
    return await call_agenticseek(prompt, provider, context)

@mcp.tool()
async def google_reasoning(prompt: str, context: str = "") -> str:
    """
    ...
    Args:
        ...
        context: Additional context
    """
    return await call_agenticseek(prompt, "google", context)

@mcp.tool()
async def smart_routing(prompt: str, priority: str = "balanced", context: str = "") -> str:
    """
    ...
    result = await call_agenticseek(prompt, provider, context)
...
# FIXED: Use a proper async-compatible main entry point
if __name__ == "__main__":
    try:
        # Log startup information
        logger.info("AgenticSeek MCP Server starting up (FIXED VERSION)")
        logger.info(f"AgenticSeek URL: {AGENTICSEEK_URL}")
        logger.info(f"Config path: {AGENTICSEEK_CONFIG_PATH}")
        
        # Check AgenticSeek availability (synchronous check for startup)
        try:
            import httpx
        ...
        # Run the MCP server (FastMCP handles the event loop properly)
        mcp.run()
    except Exception as e:
